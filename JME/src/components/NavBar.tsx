...
function NavBar({ scrollToSection }: { scrollToSection: (id: string) => void }) {
  const [activeLink, setActiveLink] = useState<string>("home");
  const [menuOpen, setMenuOpen] = useState<boolean>(false);
  const menuRef = useRef<HTMLDivElement>(null);
  const handleLinkClick = (id: string) => {
    setActiveLink(id); // Update active link
    scrollToSection(id); // Call the scrolling function
  ...
  return (
    <div className="h-20 bg-white shadow w-full fixed top-0 left-0 z-50">
      <div className="container mx-auto px-4 flex items-center justify-between  h-full">
        ...
        <div className="md:hidden">
          <button
            ...
            onClick={() => {setMenuOpen((prev=>!prev)),console.log("error")}} 
          >
            {menuOpen ? (
              <svg
      ...
      {/* Mobile Menu */}
      {menuOpen && (
        <div ref={menuRef}  className="bg-white shadow-md md:hidden absolute top-20  h-screen right-0 w-1/2 z-40">
          <div className="flex flex-col items-start py-4 space-y-4">
            <button
              onClick={() => handleLinkClick("home")}
              className={`  w-full   ${
                activeLink === "home" ? "text-white bg-blue-500 p-2 w-full text-left" : "text-gray-500 p-2 text-left"
              }`}
            >
              Home
            </button>
            <button
              onClick={() => handleLinkClick("about")}
              className={` w-full  ${
                activeLink === "about" ? "text-white bg-blue-500 p-2 w-full text-left" : "text-gray-500 p-2 text-left"
              }`}
            >
              Shop
            </button>
            <button
              onClick={() => handleLinkClick("contact")}
              className={`w-full ${
                activeLink === "contact" ? "text-white bg-blue-500 p-2 w-full text-left" : "text-gray-500  p-2 text-left"
              }`}
            >
              Contact
            </button>
